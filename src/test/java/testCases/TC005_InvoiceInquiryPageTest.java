...
public class TC005_InvoiceInquiryPageTest extends BaseClass {
    ...
    @Test(priority = 2)
    public void verifyInvoicesInquiryTab() {
        logger.info("******* Starting verifyInvoicesInquiryTab***********");
        try {
        	
        	
        	invoiceInquiry.clickOnInvoiceInquirytab();
        	logger.info("Clicked on Work Center Tab");
        	
            Assert.assertEquals(invoiceInquiry.validateInvoiceInquiryHeaderText(), "Invoices Inquiry", "Invoices Inquiry header text not diplayed");
            Assert.assertEquals(invoiceInquiry.validateInvoiceInquiryListText(), "Invoices Inquiry List", "Invoices Inquiry List text not diplayed");
    ...
    public void verifyInvoicesInquiryAdvanceFiltes() {
        ...
        try {
        	...
        	invoiceInquiry.invoiceInquiryAdvanceFilters("Accessorial", "AC01", "FL50", "Emailed To Customer", "1/4", "Acc.Invoice Processing");
        	invoiceInquiry.selectInvoiceDateRange("September 2025", "1", "September 2025", "30");
        	logger.info("Selected date range - 01/08/2025 to 30/09/2025");
        	...
        	Assert.assertTrue(invoiceInquiry.validateInvInqClearButtonDisabled(),  "Clear button should be enabled");
            Assert.assertTrue(invoiceInquiry.validateInvInqApplyButtonDisabled(),  "Apply button should be enabled");
